...
async function _initReplicas() {
  ...
  async function renderReplicas() {
    const tbody = document.getElementById('replicas-tbody');
    if (!tbody) return;
    let replicas = [];
    try { replicas = await api.listReplicas(APP_ID); } catch { return; }

    if (!replicas.length) {
      tbody.innerHTML = '<tr><td colspan="5" style="color:#8b949e;text-align:center;padding:12px">No additional replicas. Click "Add Replica" to scale out.</td></tr>';
      return;
    }

    tbody.innerHTML = replicas.map(r => {
      const statusColor = r.status === 'running' ? '#3fb950' : r.status === 'error' ? '#f85149' : '#8b949e';
      return `<tr>
        <td style="padding:8px 12px;color:#e6edf3">#${r.id}</td>
        <td style="padding:8px 12px;color:#e6edf3">${r.node_name || 'Local'}</td>
        <td style="padding:8px 12px;color:#e6edf3">${r.external_port || '—'}</td>
        <td style="padding:8px 12px"><span style="color:${statusColor};font-weight:500">${r.status}</span>${r.last_error ? `<br><span style="color:#f85149;font-size:11px">${r.last_error}</span>` : ''}</td>
        <td style="padding:8px 12px;text-align:right">
          <button class="btn-danger btn-sm replica-remove-btn" data-replica-id="${r.id}" style="padding:4px 10px;font-size:12px">Remove</button>
        </td>
      </tr>`;
    }).join('');

    tbody.querySelectorAll('.replica-remove-btn').forEach(btn => {
      btn.addEventListener('click', async () => {
        const replicaId = parseInt(btn.dataset.replicaId, 10);
        const ok = await confirm('Remove replica?', 'The replica container will be stopped and removed.');
        if (!ok) return;
        btn.disabled = true;
        btn.textContent = 'Removing…';
        try {
          await api.removeReplica(APP_ID, replicaId);
          toast('Replica removed');
          await renderReplicas();
          app = await api.getApp(APP_ID);
          updateHeaderStatus();
          renderHeader();
        } catch (e) {
  ...
  const addBtn = document.getElementById('btn-add-replica');
  if (!addBtn) return;
  ...
  addBtn.addEventListener('click', async () => {
    ...
    // Build simple modal for node selection
    const modalHtml = `
      <div style="margin-bottom:12px;font-size:14px;color:#8b949e">Choose a node for the new replica:</div>
      <select id="replica-node-select" style="width:100%;padding:8px;background:#161b22;color:#e6edf3;border:1px solid #30363d;border-radius:6px;font-size:14px">
    ...
    const confirmed = await confirm('Add Replica', modalHtml);
    if (!confirmed) return;

    const selectEl = document.getElementById('replica-node-select');
    const nodeId = selectEl && selectEl.value ? parseInt(selectEl.value, 10) : null;

    addBtn.disabled = true;
    addBtn.textContent = 'Starting…';
    try {
      await api.scaleApp(APP_ID, { node_id: nodeId });
      toast('Replica started');
      await renderReplicas();
      app = await api.getApp(APP_ID);
      updateHeaderStatus();
      renderHeader();
      ...
      addBtn.textContent = 'Add Replica';
    }
  });
}
